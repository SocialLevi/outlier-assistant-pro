            ...
            <div className="py-20 px-8">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {projects.map((project, index) => (
                        <motion.div
                            key={project.name}
                            initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }}
                        ...
                        >
                            ...
                            <div className="mt-6">
                                <motion.button
                                    ...
                                    whileHover={{ y: -2, boxShadow: "0 10px 20px -10px rgba(168, 85, 247, 0.5)" }}
    ...
    return (
        <Section id="contact-us" setActivePage={setActivePage}>
            <div className="py-20 px-8">
                ...
                <motion.form 
                    initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.7 }}
                    className="space-y-6 max-w-xl mx-auto bg-white border border-gray-200/80 rounded-xl p-8 shadow-lg">
                    <div>
                    ...
                    <div>
                        <motion.button whileHover={{ scale: 1.05, y: -2, boxShadow: "0 10px 20px -10px rgba(168, 85, 247, 0.5)" }} whileTap={{ scale: 0.95 }} type="submit" className="w-full py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-gradient-to-r from-purple-600 to-cyan-600">
                            Send Message
                        </motion.button>
                    </div>
                </motion.form>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }}
                        className="fixed bottom-24 right-4 sm:right-8 w-[calc(100%-2rem)] sm:w-96 h-[70%] sm:h-[500px] bg-white/80 backdrop-blur-xl border border-gray-200/50 rounded-2xl shadow-2xl flex flex-col z-40">
    ...
    <AnimatePresence>
        {show && (
            <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 50 }}
                className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-800 text-white px-6 py-3 rounded-full shadow-lg"
            >
                {message}
            </motion.div>
        )}
...
export default function App() {
    ...
    const [projects] = useState(initialProjects.map(p => ({...p, status: "Active", tasks: Math.floor(Math.random() * 5000) + 1000 })));
    ...
    const NavLink = ({ page, id, icon: Icon, isMobile = false }) => (
        ...
        >
            {activePage === id && !isMobile && (
                <motion.div layoutId="active-nav-link" className="absolute inset-0 bg-gray-100 rounded-lg"
